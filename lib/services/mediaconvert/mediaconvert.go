...
import (
	...
	awsMediaConvert "github.com/aws/aws-sdk-go/service/mediaconvert"
	"os"
)
...
type config struct {
	jobTemplate  string
	mediaConvert *awsMediaConvert.MediaConvert
}
...
func NewService() (Service, error) {
	...
	return &config{
		...
		mediaConvert: awsMediaConvert.New(sess),
	}, nil
}

func (c *config) CreateJob(ctx context.Context, bucket, objectKey string) (*string, error) {
	if output, err := c.mediaConvert.CreateJobWithContext(ctx, &awsMediaConvert.CreateJobInput{
